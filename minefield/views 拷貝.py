...
def init_questionnaire():
	...
	tiebreakers = Tiebreaker.objects.all()
	tiebreaker_list = list(tiebreakers.values())
	...
	for t in tiebreaker_list:
		print('t=' + str(t))
		id = t['id']
		del t['id']
		entry = copy.deepcopy(t)
		tiebreaker_dict[id] = entry
		tiebreaker_dict[id].update({ 'show_q': False, 'disabled_ans': [], 'show_dkn': False })
	# print('question_list=' + str(question_list))
	print('tiebreaker_dict=' + str(tiebreaker_dict))
...
def show_questions_s(request, student_name):
	global question_dict
	global tiebreaker_dict
	global tiebreaker_index
	question_list = convert_dict_to_list(question_dict)
	tiebreaker = copy.deepcopy(tiebreaker_dict[tiebreaker_index])
	team_members = get_team_members(student_name)
	count = 0									# num of questions for which team eligible to guess
	for q in question_list:
...
def update_master_question(request, teacher_name):
	global question_dict
	if request.method == 'POST':
		question_id = int(request.POST['question_id'])
		action = request.POST['action']
		if action == 'show':
			question_dict[question_id]['show_q'] = True
		elif action == 'hide':
			question_dict[question_id]['show_q'] = False
		else:
			print('In update_master_question()..no action returned with post')	
	return HttpResponseRedirect(reverse('minefield:master', args=(teacher_name,)))
		

def update_master_answers(request, teacher_name):
	global question_dict
	if request.method == 'POST':
		question_id = int(request.POST['question_id'])
		...
		for a in ans:
			if a in checked:
				disable_answer(question_id, a)
			else:
				enable_answer(question_id, a)
	return HttpResponseRedirect(reverse('minefield:master', args=(teacher_name,)))


def update_master_discussion(request, teacher_name):
	global question_dict
	if request.method == 'POST':
		question_id = int(request.POST['question_id'])
		action = request.POST['action']	
		if action == 'hide':
			question_dict[question_id]['show_dkn'] = False
		elif action == 'show':
			question_dict[question_id]['show_dkn'] = True
		else:
			print('In update_master_discussion()..no action returned with post')
	return HttpResponseRedirect(reverse('minefield:master', args=(teacher_name,)))
